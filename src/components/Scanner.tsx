...
export default function Scanner({ onEventsDetected, isProcessing }: ScannerProps) {
  ...
  const captureFrame = async () => {
    ...
    const now = Date.now();
    if (now - lastProcessedTime < 2000) return; // Prevent processing more often than every 2 seconds
  ...
  useEffect(() => {
    ...
    const interval = setInterval(captureFrame, 500); // Check more frequently but process based on lastProcessedTime
